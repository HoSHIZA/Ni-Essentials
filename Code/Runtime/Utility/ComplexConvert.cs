    ...
    public static class ComplexConvert
    {
        ...
        public static object ChangeType(object obj, Type type)
        {
            switch (obj)
            {
                ...
                case IConvertible:  return Convert.ChangeType(obj, type);
            }
            
            var method = obj.GetType()
                .GetMethods(BindingFlags.Public | BindingFlags.Static)
                .FirstOrDefault(t => t.Name is "op_Implicit" or "op_Explicit" && t.ReturnType == type);
            ...
            if (method != null)
            {
                ...
                return method.Invoke(null, _cachedConvertParameter);
            }
